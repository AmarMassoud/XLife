...
public class LifeXMainMenu extends Menu {
    private final LifeX plugin = LifeX.getPlugin(LifeX.class);
    public LifeXMainMenu(Player p) {
        super(45, "&b&lLifeX Main Menu");

        setItem(10, ItemCreator.quickBuild(new ItemStack(XMaterial.BEDROCK.parseItem()), "&bAmount of hearts on first join", Arrays.asList("&eClick me to change the amount of hearts a player starts with when they first join the server.")), event -> {
            p.closeInventory();
        ...
        setItem(13, ItemCreator.quickBuild(new ItemStack(XMaterial.RED_STAINED_GLASS_PANE.parseItem()), "&bAmount of hearts increased on death", Arrays.asList("&eClick me to change the amount of hearts", "&ethat will be added to a player on death.")), event -> {
            p.closeInventory();
        ...
        setItem(16, ItemCreator.quickBuild(new ItemStack(XMaterial.RED_STAINED_GLASS.parseItem()), "&bAdd player to the LifeX whitelist.", Arrays.asList("&eClick me to add a player to the LifeX whitelist")), event -> {
            p.closeInventory();
        ...
        setItem(25, ItemCreator.quickBuild(new ItemStack(XMaterial.RED_WOOL.parseItem()), "&bRemove player from the LifeX whitelist.", Arrays.asList("&eClick me to remove a player from the LifeX whitelist")), event -> {
            p.closeInventory();
